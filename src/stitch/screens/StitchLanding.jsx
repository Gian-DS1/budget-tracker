...
const FEATURES = [
  ...
  { icon: 'credit_card', title: 'Tarjetas de crédito', desc: 'Ciclos de corte y pago, cashback por categoría y saldo sin sorpresas.' },
...
const STATS = [
  { value: 37, suffix: '', label: 'Categorías listas' },
  { value: 11, suffix: '', label: 'Pantallas de control' },
  { value: 1, prefix: '', suffix: '%', label: 'Cashback rastreado' },
  { value: 100, suffix: '%', label: 'Pensada para RD' },
];
    ...
    <div className="glass-card rounded-lg p-lg inner-glow">
      <div className="flex items-center justify-between mb-lg">
        ...
        <span className="flex items-center gap-xs font-mono-data text-mono-data text-secondary uppercase">
          <span className="w-1.5 h-1.5 rounded-full bg-secondary status-glow-live" /> En vivo
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-sm">
        {[
          ['Ingresos', '+170K', 'text-tertiary'],
          ['Gastos', '−44.5K', 'text-accent-error'],
          ['Ahorro', '73.8%', 'text-secondary'],
        ].map(([l, v, c]) => (
...
function HealthMockup() {
  // Anillo SVG (87 de salud). r=52, circunferencia ≈ 326.7.
  const pct = 0.87;
  const C = 2 * Math.PI * 52;
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-lg">
        <span className="font-mono-data text-mono-data text-text-muted uppercase tracking-widest">Salud financiera</span>
        <span className="font-mono-data text-mono-data text-tertiary uppercase">Excelente</span>
      </div>
      <div className="flex items-center gap-lg flex-grow">
        <div className="relative shrink-0" style={{ width: 132, height: 132 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="font-mono-data text-mono-data text-text-muted uppercase mt-xs">de 100</span>
          </div>
        </div>
        <div className="min-w-0 flex-grow space-y-sm">
          {[
            ['Presupuesto base cero', 'Asignado', 'text-tertiary'],
            ['Tarjetas al día', '0 vencidas', 'text-secondary'],
            ['Tasa de ahorro', '73.8%', 'text-primary'],
          ].map(([l, v, c]) => (
            <div key={l} className="flex items-center justify-between border-b border-border-subtle pb-sm last:border-0 last:pb-0">
              <span className="font-body-md text-body-md text-on-surface-variant truncate">{l}</span>
              <span className={`font-headline-md text-[15px] tracking-tight ${c}`}>{v}</span>
            </div>
          ))}
        </div>
      </div>
...
function BudgetMockup() {
  const rows = [
    ['Alquiler', 100, 'bg-tertiary'],
    ['Supermercado', 64, 'bg-primary'],
    ['Transporte', 38, 'bg-secondary'],
  ];
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-md">
        <span className="font-mono-data text-mono-data text-text-muted uppercase tracking-widest">Presupuesto</span>
        <span className="font-mono-data text-mono-data text-tertiary uppercase">Base cero</span>
      </div>
...
function LedgerMockup() {
  const rows = [
    ['shopping_cart', 'Supermercado Nacional', '−RD$ 4,250', 'text-accent-error'],
    ['payments', 'Salario quincenal', '+RD$ 85,000', 'text-tertiary'],
    ['local_gas_station', 'Gasolina', '−RD$ 1,800', 'text-accent-error'],
  ];
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-md">
        <span className="font-mono-data text-mono-data text-text-muted uppercase tracking-widest">Movimientos</span>
        <span className="font-mono-data text-mono-data text-secondary uppercase">Hoy</span>
      </div>
        ...
        <div className="max-w-[1100px] mx-auto px-md sm:px-margin-safe py-xl">
          ...
          >
            ...
            >
              ...
              <h3 className="font-headline-md text-[18px] text-on-surface tracking-tight mb-xs">Presupuesto base cero</h3>
              <p className="font-body-md text-body-md text-on-surface-variant">Asigna cada peso hasta llegar a cero y sabe exactamente cuánto puedes gastar.</p>
            </motion.div>
        ...
        >
          ...
          <motion.div variants={v.inItem}>
            <PrimaryCTA onClick={onAccess} snap={v.snap} className="px-xl py-md">
              <MS name="rocket_launch" className="text-[18px]" /> Crear cuenta gratis
            </PrimaryCTA>
          </motion.div>
        </motion.div>
      </section>
