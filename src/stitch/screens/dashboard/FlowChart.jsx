  ...
  const SelDot = (props) => {
    ...
    return (
      <g style={{ pointerEvents: 'none' }}>
        ...
        <circle cx={cx} cy={cy} r={3.5} fill={color} stroke="#101112" strokeWidth={1.5} />
      </g>
    );
  };
  return SelDot;
      ...
      <div className="flex-grow min-h-[190px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={series} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            ...
            {selPoint && (
              <ReferenceLine x={selPoint.label} stroke="#454655" strokeDasharray="3 3" />
            )}
            ...
            <Line type="monotone" dataKey="inc" stroke={INC} strokeWidth={2} dot={makeSelDot(selY, selM, INC)} activeDot={{ r: 4, fill: INC, stroke: '#101112', strokeWidth: 1.5 }} isAnimationActive animationDuration={600} animationEasing="ease-out" />
            <Line type="monotone" dataKey="exp" stroke={EXP} strokeWidth={2} dot={makeSelDot(selY, selM, EXP)} activeDot={{ r: 4, fill: EXP, stroke: '#101112', strokeWidth: 1.5 }} isAnimationActive animationDuration={600} animationEasing="ease-out" />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
