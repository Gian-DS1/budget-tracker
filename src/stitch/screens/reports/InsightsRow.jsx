...
export default function InsightsRow({ insights }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-md">
      <InsightCard label={t('pages.analysis') || 'Gasto mensual promedio'} icon="trending_flat" value={fmt(avgMonthlyExpense)} sub={t('pages.loadingData') || 'en el período'} />
      <InsightCard label={t('pages.trends') || 'Mes de mayor gasto'} icon="trending_up" cls="text-accent-warning"
        value={topMonth ? topMonth.label : '—'} sub={topMonth ? fmt(topMonth.amount) : t('pages.noData')} />
      <InsightCard label={t('pages.insights') || 'Categoría más cara'} icon="local_fire_department" cls="text-accent-error"
